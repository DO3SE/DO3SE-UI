...
class SavePanel(wx.Panel):
    def __init__(self, app, parent, results):
        ...
        self.slOutputs.SetAvailable(
            [(x['long'], x['variable']) for x in model.output_fields.values()])

        # "Include column headers?"
        sHeaders = wx.BoxSizer(wx.HORIZONTAL)
        s.Add(sHeaders, 0, wx.ALL | wx.ALIGN_LEFT, 5)
        self.chkOutputHeaders = wx.CheckBox(
            self, label="Include column headers")
        sHeaders.Add(self.chkOutputHeaders, 0, wx.EXPAND)
        self.chkOutputHeaders.SetValue(True)

        self.chkReduceOutput = wx.CheckBox(
            self, label="Only data from during growing season")
    ...
    def set_values(self, values):
        self.slOutputs.SetSelection(
            (model.output_fields[f]['long'] for f in values['fields']))
