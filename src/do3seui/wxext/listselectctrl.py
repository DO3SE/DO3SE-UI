...
class SaveListSelectCtrl(wx.Panel):
    """
    Given a list of items, provides a userinterface for selecting a subset of
    them in any order.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # weepanel = wx.Panel(self)
        # weesizer = wx.BoxSizer(wx.HORIZONTAL)
        # weesizer.Add(weepanel, 0, wx.EXPAND)

        # selectAll = wx.CheckBox(self, label="Select All")
        # weepanel.Add(selectAll, 0, wx.EXPAND)

        # Available entries
        s1 = wx.BoxSizer(wx.VERTICAL)
        mainsizer.Add(s1, 1, wx.EXPAND)
        s1.Add(wx.StaticText(self, label = 'Available fields'))
        self.list_avail = wx.ListBox(self, style = wx.LB_MULTIPLE)
        s1.Add(self.list_avail, 1, wx.EXPAND)

        # Add/remove/up/down
        s2 = wx.BoxSizer(wx.VERTICAL)
        mainsizer.Add(s2, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 4)
        self.selectAll = wx.CheckBox(self, label="Select All")
        s2.Add(self.selectAll, 0, wx.EXPAND)
        s2.AddSpacer(4)
        self.button_add = wx.Button(self, wx.ID_ADD, 
                style = wx.BU_EXACTFIT)
        s2.Add(self.button_add, 0, wx.EXPAND)
        ...
        self.Bind(wx.EVT_CHECKBOX, self.OnCheck, self.selectAll)
        self.Bind(wx.EVT_BUTTON, self.OnAdd, self.button_add)
        self.Bind(wx.EVT_BUTTON, self.OnRemove, self.button_remove)
        self.Bind(wx.EVT_BUTTON, self.OnUp, self.button_up)
        self.Bind(wx.EVT_BUTTON, self.OnDown, self.button_down)
    ...
    def OnCheck(self, evt):
        if self.selectAll.GetValue():
            for i in range(self.list_avail.GetCount()):
                
                self.list_avail.SetSelection(i)
        else:
            for i in range(self.list_avail.GetCount()):
                
                self.list_avail.Deselect(i)
    ...
    def OnAdd(self, evt):
        sels = self.list_avail.GetSelections()
        for sel in sels:
            try:
                if sel != -1:
