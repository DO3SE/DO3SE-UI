...
input_fields = dicts_to_map(to_dicts(('module', 'variable', 'type', 'required', 'short', 'long'), (
    ...
    (inputs,    'ts_c',     float,  True,
     'Ts_C (C)',     'Temperature (Ts_C, Celcius) *'),
    (inputs,    'tleaf',    float,  False,  'Tleaf (C)',
     'Leaf temperature (Tleaf, Celcius)'),
    (inputs,    'vpd',      float,  True,   'VPD (kPa)',
     'Vapour Pressure Deficit (VPD, kPa) *'),
    (inputs,    'uh_zr',    float,  True,   'uh_zR (m/s)',
     'Measured wind speed (uh_zR, m/s) *'),
    (inputs,    'precip',   float,  False,
     'precip (mm)',  'Precipitation (precip, mm) *'),
    (inputs,    'p',        float,  True,
     'P (kPa)',      'Pressure (P, kPa) *'),
    (inputs,    'o3_ppb_zr', float,  True,   'O3_zR (ppb)',
     'Measured O3 density (O3_zR, ppb) *'),
    (inputs,    'co2',      float,  False,  'CO2 (ppm)',
     u'Ambient CO2 concentration (CO2, ppm)'),
    (inputs,    'hd',       float,  False,  'Hd (Wh/m^2)',
     u'Sensible heat flux (Hd, Wh/m\u00b2)'),
    (inputs,    'r',        float,  False,  'R (Wh/m^2)',
...
output_fields = dicts_to_map(to_dicts(('module', 'variable', 'type', 'short', 'long'), (
    ...
    (inputs,    'cloudfrac',       float,
     'cloudfrac',  u'cloudfrac [fraction] **'),
    (inputs,        'hr',       int,
     'Hour',             'Hour of day (0 to 23)'),
    (inputs,        'ts_c',     float,  'Ts_C (C)',
     'Temperature (Ts_C, Celcius)'),
    (inputs,        'tleaf',    float,  'Tleaf (C)',
     'Leaf temperature (Tleaf, Celcius)'),
    (inputs,        'vpd',      float,  'VPD (kPa)',
     'Vapour Pressure Deficit (VPD, kPa)'),
    (inputs,        'uh_zr',    float,  'uh_zR (m/s)',
     'Measured wind speed (uh_zR, m/s)'),
    (inputs,        'precip',   float,
     'precip (mm)',      'Precipitation (precip, mm)'),
    (inputs,        'precip_acc', float, 'precip_acc',
     'Accumulated precipitation (precip_acc, m)'),
    (inputs,        'p',        float,  'P (kPa)',          'Pressure (P, kPa)'),
    (inputs,        'o3_ppb_zr', float,  'O3_zR (ppb)',
     'Measured O3 density (O3_zR, ppb)'),
    (inputs,        'co2',      float,  'CO2 (ppm)',
     u'Ambient CO2 concentration (CO2, ppm)'),
    (inputs,        'hd',       float,  'Hd (Wh/m^2)',
     u'Sensible heat flux (Hd, Wh/m\u00b2)'),
    (inputs,        'r',        float,  'R (Wh/m^2)',
     u'Global radiation (R, Wh/m\u00b2)'),
    ...
    (inputs,        'uh',       float,  'uh (m/s)',
     'Wind speed at target canopy (uh, m/s)'),
    (inputs,        'rn',       float,  'Rn (MJ/m^2)',
    ...
    (variables,     'ra_tar_i',       float,  'Ra_tar_i (s/m)',
     'Aerodynamic resistance to target canopy(Ra, s/m)'),
    (variables,     'ra_ref_i',       float,  'Ra_ref_i (s/m)',
     'Aerodynamic resistance to reference canopy(Ra, s/m)'),
    (variables,     'rb',       float,  'Rb (s/m)',
     'Boundary layer resistance (Rb, s/m)'),
    (variables,     'rsur',     float,  'Rsur (s/m)',
     'Surface resistance (Rsur, s/m)'),
    (variables,     'rinc',     float,  'Rinc (s/m)',
     'In-canopy resistance (Rinc, s/m)'),
    (variables,     'rsto',     float,  'Rsto (s/m)',
     'Mean stomatal resistance (Rsto, s/m)'),
    (variables,     'gsto',     float,  'Gsto (mmol/m^2/s)',
    ...
    (variables,     'gsun_l',   float,  'Gsun_l (mmol/m^2/s)',
     u'Leaf stomatal conductance sunlit (Gsun_l, mmol/m\u00b2/s)'),
    (variables,     'gsun_l_ms',   float,  'Gsun_l (m/s)',
     u'Leaf stomatal conductance sunlit (Gsun_l, m/s)'),
    (variables,     'rsto_c',   float,  'Rsto_c (s/m)',
     'Canopy stomatal resistance (Rsto_c, s/m)'),
    (variables,     'gsto_c',   float,  'Gsto_c (mmol/m^2/s)',
     u'Canopy stomatal conductance (Gsto_c, mmol/m\u00b2/s)'),
    (variables,     'rgs',      float,  'Rgs (s/m)',
     'Ground surface resistance (Rgs, s/m)'),
    (variables,     'vd',       float,  'Vd (m/s)',
     'Deposition velocity (Vd, m/s)'),
    (variables,     'o3_ppb_i', float,  'O350 (ppb)',
     'Ozone concentration at 50m (O350, ppb)'),
    (variables,     'o3_ppb',   float,  'O3 (ppb)',
     'Ozone concentration at canopy (O3, ppb)'),
    (variables,     'o3_nmol_m3', float, 'O3 (nmol/m^3)',
     u'Ozone concentration at canopy (O3, nmol/m\u00b3)'),
    (variables,     'fst',      float,  'Fst (nmol/m^2/s)',
     u'Upper leaf stomatal O3 flux (Fst, nmol/m\u00b2/s)'),
    (variables,     'fst_sun',      float,  'Fst Sunlit (nmol/m^2/s)',
     u'Upper leaf stomatal sunlit O3 flux (Fst, nmol/m\u00b2/s)'),
    (variables,     'afst0',    float,  'POD0 (mmol/m^2 PLA)',
     u'Accumulated Fst (POD0, mmol/m\u00b2 PLA)'),
    (variables,     'afsty',    float,  'PODY (mmol/m^2 PLA)',
     u'Accumulated sunlit Fst over threshold Y (PODY, mmol/m\u00b2 PLA)'),
    (variables,     'afsty_total',    float,  'PODY_total (mmol/m^2 PLA)',
     u'Accumulated Fst over threshold Y (PODY_total, mmol/m\u00b2 PLA)'),
    (variables,     'ftot',     float,  'Ftot (nmol/m^2/s)',
     u'Total ozone flux (Ftot, nmol/m\u00b2/s)'),
    (variables,     'ot40',     float,  'OT40 (ppm)',
     'Ozone over 40 ppb (OT40, ppm)'),
    (variables,     'aot40',    float,  'AOT40 (ppm)',
     'Accumulated OT40 over growth period (AOT40, ppm)'),

    (variables,     'lai',      float,  'LAI',
     u'Leaf Area Index (LAI, m\u00b2/m\u00b2)'),
    (variables,     'sai',      float,  'SAI',
     u'Stand Area Index (SAI, m\u00b2/m\u00b2)'),

    (variables,     'pet',      float,  'PEt (m/day)',
     'Potential plant transpiration (PEt, m/day)'),
    (variables,     'et',       float,  'Et (m/day)',
     'Plant traspiration (Et, m/day)'),
    (variables,     'ei',       float,  'Ei (m/day)',
     'Evaporation of intercepted precipitation (Ei, m/day)'),
    (variables,     'es',       float,  'Es (m/day)',
     'Soil evaporation (Es, m/day)'),
    (variables,     'sn',       float,  'Sn (m^3/m^3)',
     u'Soil water content (Sn, m\u00b3/m\u00b3)'),
    (variables,     'per_vol',  float,  'per_vol (%)',
     'Volumetric water content (per_vol, %)'),
    (variables,     'smd',      float,  'SMD (m)',
     'Soil moisture deficit (SMD, m)'),
    (variables,     'swp',      float,  'SWP (MPa)',
     'Soil water potential (SWP, MPa)'),
    (variables,     'lwp',      float,  'LWP (MPa)',
     'Leaf water potential (LWP, MPa)'),
    (variables,     'asw',      float,  'ASW (m^3/m^3)',
     u'Available soil water (ASW, m\u00b3/m\u00b3)'),

    (variables,     'sn_meas',  float,  'Sn_meas (m^3/m^3)',
     u'Soil water content at measurement depth (Sn_meas, m\u00b3/m\u00b3)'),
    (variables,     'swp_meas', float,  'SWP_meas (MPa)',
     'Soil water potential at measurement depth (SWP_meas, MPa)'),
    (variables,     'smd_meas', float,  'SMD_meas (m)',
     'Soil moisture deficit at measurement depth (SMD_meas, m)'),

    (variables,     'fphen',    float,  'f_phen',
     'Phenology effect on stomatal conductance (f_phen, fraction)'),
    (variables,     'leaf_fphen', float, 'leaf_f_phen',
     'Phenology effect on leaf stomatal conductance (leaf_f_phen, fraction)'),
    (variables,     'flight',   float,  'f_light',
     'Irradiance effect on stomatal conductance (f_light, fraction)'),
    (variables,     'flightsun',   float,  'Flightsun',
     'Sunlit Irradiance effect on stomatal conductance (Flightsun, fraction)'),
    (variables,     'flightshade',   float,  'Flightshade',
     'Sunlit Irradiance effect on stomatal conductance (Flightshade, fraction)'),
    (variables,     'leaf_flight', float, 'leaf_f_light',
     'Irradiance effect on leaf stomatal conductance (leaf_f_light, fraction)'),
    (variables,     'ftemp',    float,  'f_temp',
     'Temperature effect on stomatal conductance (f_temp, fraction)'),
    (variables,     'fvpd',     float,  'f_VPD',
     'VPD effect on stomatal conductance (f_VPD, fraction)'),
    (variables,     'fxwp',     float,  'f_SW',
     'Soil water effect on stomatal conductance (f_SW, fraction)'),
    (variables,     'fo3',      float,  'f_O3',
     'O3 effect on stomatal conductance (f_O3, fraction)'),

    # Debug variables
    (pn_gsto,       'gsto_final', float, 'gsto_final',
     '[DEBUG] Photosynthetic Gsto (base)'),
    (pn_gsto,       'pngsto_l', float,  'Pn Gsto_l',
     '[DEBUG] Photosynthetic Gsto (leaf)'),
    (pn_gsto,       'pngsto',   float,  'Pn Gsto',
     '[DEBUG] Photosynthetic Gsto (mean)'),
    (pn_gsto,       'pngsto_c', float,  'Pn Gsto_c',
     '[DEBUG] Photosynthetic Gsto (canopy)'),
    (pn_gsto,       'pngsto_pet', float, 'Pn Gsto_PEt',
    ...
    (variables,     'ppardif',  float,
     'pPARdiff',         '[DEBUG] pPARdiff'),
    (variables,     'fpardir',  float,  'fPARdir',          '[DEBUG] fPARdir'),
    (variables,     'fpardif',  float,
     'fPARdiff',         '[DEBUG] fPARdiff'),
    (variables,     'laisun',   float,  'LAIsun',           '[DEBUG] LAIsun'),
    (variables,     'laishade', float,  'LAIshade',         '[DEBUG] LAIshade'),
    (variables,     'parsun',   float,
     'PARsun',           '[DEBUG] PARsun'),
    (variables,     'parshade', float,
     'PARshade',         '[DEBUG] PARshade'),
    ...
    (soilwater,     'et_hr',       float,  'Et hr (m/hour)',
     'Plant traspiration (Et, m/day)'),
    (soilwater,     'ei_hr',       float,  'Ei hr (m/hour)',
     'Evaporation of intercepted precipitation (Ei, m/day)'),
    (soilwater,     'es_hr',       float,
     'Es hr (m/hour)',       'Soil evaporation (Es, m/day)'),
    (variables,     'es_blocked',       bool,
...
paramdefs = dicts_to_map(to_dicts(('group', 'variable', 'cls', 'args', 'name', 'contexthelp'), (
    ...
    ('siteloc', 'lat', FloatSpinField, (-90, 90, 50, 0.1, 3),
     'Latitude (decimal degrees North)', ''),
    ('siteloc', 'lon', FloatSpinField, (-180, 180, 0, 0.1, 3),
    ...
    ('siteloc', 'soil_tex', ChoiceField,
     (soil_classes, default_soil_class), 'Soil texture', ''),
    ...
    ('meas', 'd_meas', FloatSpinField, (0.1, 2, 0.5, 0.1, 1),
     'Soil water measurement depth (m)', ''),

    ('vegchar', 'h', FloatSpinField,
     (0.1, 100, 25, 0.1, 1), 'Canopy height (h, m)', ''),
    ('vegchar', 'root', FloatSpinField,
     (0.1, 10, 1.2, 0.1, 1), 'Root depth (root, m)', ''),
    ('vegchar', 'lm', FloatSpinField,
     (0.01, 1, 0.05, 0.01, 2), 'Leaf dimension (Lm, m)', ''),
    ('vegchar', 'albedo', FloatSpinField,
     (0.01, 0.99, 0.12, 0.01, 2), 'Albedo (fraction)', ''),
    ('vegchar', 'gmax', SpinField, (1, 10000, 148),
     u'gmax (mmol O3/m\u00b2 PLA/s)', ''),
    ('vegchar', 'gmorph', FloatSpinField,
     (0.01, 1, 1, 0.01, 2), 'Sun/shade factor (fraction)', ''),
    ('vegchar', 'fmin', FloatSpinField,
     (0.01, 0.99, 0.13, 0.01, 2), 'fmin (fraction)', ''),
    ('vegchar', 'rext', SpinField, (0, 20000, 2500),
     'External plant cuticle resistance (Rext, s/m)', ''),
    ('vegchar', 'y', FloatSpinField, (0.1, 100, 1.0, 0.1, 1),
     u'Threshold Y for PODy (nmol/m\u00b2/s)', ''),
    ('vegchar', 'g_sto_0', SpinField, (1, 100000, 30000),
     u'Closed stomata conductance (gsto0, umol/m\u00b2/s)', ''),
    ('vegchar', 'm', FloatSpinField, (0.01, 30.0, 16.83, 0.1, 2),
     u'Species-specific sensitivity to An (m, dimensionless)', ''),
    ('vegchar', 'v_cmax_25', FloatSpinField, (0.01, 500.0, 30.0, 0.1, 2),
        u'Maximum catalytic rate at 25\u00b0C (Vcmax, umol/m\u00b2/s)', ''),
    ('vegchar', 'j_max_25', FloatSpinField, (0.01, 1000.0, 60.0, 0.1, 2),
        u'Maximum rate of electron transport at 25\u00b0C (Jmax, umol/m\u00b2/s)', ''),

    ('vegenv', 'f_lightfac', FloatSpinField,
     (0.001, 0.999, 0.006, 0.001, 3), 'light_a', ''),
    ('vegenv', 't_min', SpinField, (-10, 100, 0),
     u'Minimum temperature (T_min, \u00b0C)', ''),
    ('vegenv', 't_opt', SpinField, (-10, 100, 21),
     u'Optimum temperature (T_opt, \u00b0C)', ''),
    ('vegenv', 't_max', SpinField, (-10, 100, 35),
     u'Maximum temperature (T_max, \u00b0C)', ''),
    ('vegenv', 'vpd_max', FloatSpinField, (0, 100, 1,
                                           0.01, 2), 'VPD for max. g (VPD_max, kPa)', ''),
    ('vegenv', 'vpd_min', FloatSpinField, (0, 100, 3.25,
                                           0.01, 2), 'VPD for min. g (VPD_min, kPa)', ''),
    ('vegenv', 'vpd_crit', FloatSpinField, (0, 1000, 1000, 1, 1),
     'Critical daily VPD sum (VPD_crit, kPa)', ''),
    ('vegenv', 'swp_min', FloatSpinField, (-6, 0, -1.25,
                                           0.01, 2), 'SWP for min. g (SWP_min, MPa)', ''),
    ...
    ('modelopts', 'tleaf', ChoiceField, (tleaf_calcs,
                                         default_tleaf_calc), 'Leaf temperature calculation', ''),
    ('modelopts', 'fo3', ChoiceField,
     (fO3_calcs, default_fO3_calc), 'fO3 calculation', ''),
    ('modelopts', 'fxwp', ChoiceField, (fXWP_calcs,
                                        default_fXWP_calc), 'Soil water influence on Gsto', ''),
    ('modelopts', 'lwp', ChoiceField,
    ...
    ('season', 'sgs', SpinField, (1, 730, 121),
     'Start of growing season (SGS, day of year)', ''),
    ('season', 'egs', SpinField, (1, 730, 273),
     'End of growing season (EGS, day of year)', ''),
    ('season', 'mid_anthesis', SpinField,
     (1, 730, 161), 'Mid-anthesis (day of year)', ''),
    ('season', 'lai_a', FloatSpinField, (0, 20, 0, 0.1, 1),
     u'LAI at SGS (LAI_a, m\u00b2/m\u00b2)', ''),
    ('season', 'lai_b', FloatSpinField, (0, 20, 4, 0.1, 1),
     u'First mid-season LAI (LAI_b, m\u00b2/m\u00b2)', ''),
    ('season', 'lai_c', FloatSpinField, (0, 20, 4, 0.1, 1),
     u'Second mid-season LAI (LAI_c, m\u00b2/m\u00b2)', ''),
    ('season', 'lai_d', FloatSpinField, (0, 20, 0, 0.1, 1),
     u'LAI at EGS (LAI_d, m\u00b2/m\u00b2)', ''),
    ('season', 'lai_1', SpinField, (1, 250, 30),
     'Period from LAI_a to LAI_b (LAI_1, days)', ''),
    ('season', 'lai_2', SpinField, (1, 250, 30),
     'Period from LAI_c to LAI_d (LAI_2, days)', ''),
    ('season', 'sai', ChoiceField, (SAI_calcs,
                                    default_SAI_calc), 'SAI calculation', ''),

    ('fphen', 'fphen_a', FloatSpinField,
     (0, 1, 0, 0.1, 1), 'Fphen at SGS (fphen_a)', ''),
    ('fphen', 'fphen_b', FloatSpinField, (0, 1, 1, 0.1, 1),
     'First mid-season Fphen (fphen_b)', ''),
    ('fphen', 'fphen_c', FloatSpinField, (0, 1, 1, 0.1, 1),
     'Second mid-season Fphen (fphen_c)', ''),
    ('fphen', 'fphen_d', FloatSpinField, (0, 1, 1, 0.1, 1),
     'Third mid-season Fphen (fphen_d)', ''),
    ('fphen', 'fphen_e', FloatSpinField,
     (0, 1, 0, 0.1, 1), 'Fphen at EGS (fphen_e)', ''),
    ('fphen', 'fphen_1', SpinField, (0, 200, 15),
     'Period from fphen_a to fphen_b (fphen_1, days)', ''),
    ('fphen', 'fphen_lima', SpinField, (0, 730, 0),
     'Start of SWP limitation (fphen_limA, day of year)', ''),
    ('fphen', 'fphen_2', SpinField, (0, 200, 1),
     'Period from fphen_b to fphen_c (fphen_2, days)', ''),
    ('fphen', 'fphen_3', SpinField, (0, 200, 1),
     'Period from fphen_c to fphen_d (fphen_3, days)', ''),
    ('fphen', 'fphen_limb', SpinField, (0, 730, 0),
     'End of SWP limitation (fphen_limB, day of year)', ''),
    ('fphen', 'fphen_4', SpinField, (0, 200, 20),
     'Period from fphen_d to fphen_e (fphen_4, days)', ''),

    ('leaf_fphen', 'leaf_fphen', ChoiceField, (leaf_fphen_calcs,
                                               default_leaf_fphen_calc), 'Leaf fphen calculation', ''),
    ('leaf_fphen', 'astart', SpinField, (1, 730, 153),
     'Start of O3 accumulation (Astart, day of year)', ''),
    ('leaf_fphen', 'aend', SpinField, (1, 730, 208),
     'End of O3 accumulation (Aend, day of year)', ''),
    ('leaf_fphen', 'leaf_fphen_a', FloatSpinField,
     (0, 1, 0, 0.1, 1), 'Leaf fphen at Astart (leaf_fphen_a)', ''),
    ('leaf_fphen', 'leaf_fphen_b', FloatSpinField,
     (0, 1, 1, 0.1, 1), 'Leaf fphen mid-season (leaf_fphen_c)', ''),
    ('leaf_fphen', 'leaf_fphen_c', FloatSpinField,
     (0, 1, 0, 0.1, 1), 'Leaf fphen at Aend (leaf_fphen_e)', ''),
    ('leaf_fphen', 'leaf_fphen_1', SpinField, (0, 300, 15),
     'Period from leaf_fphen_a to leaf_fphen_b (days)', ''),
    ('leaf_fphen', 'leaf_fphen_2', SpinField, (0, 300, 30),
     'Period from leaf_fphen_b to leaf_fphen_c (days)', ''),
)), 'variable', OrderedDict)
